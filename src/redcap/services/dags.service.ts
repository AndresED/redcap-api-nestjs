...
dotenv.config();
@Injectable()
export class DagsRedcapService {
    token: string;
    constructor(
    ...
    import(fileData: UploadFileDto) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const data = `token=${this.token}&data=${xlData}&content=dag&format=json&returnFormat=json`;
                const config = {
                    headers: {
    ...
    export() {
        return new Promise(async (resolve, reject) => {
            try {
                const url = process.env.REDCAP_HOST + process.env.REDCAP_PATH;
                const data = `token=${this.token}&content=dag&format=json&returnFormat=json`;
                const config = {
                    headers: {
                        'Content-Type': `application/x-www-form-urlencoded`,
                        'Accept': 'application/json'
    ...
    delete(dags: DagsDeleteDto) {
        return new Promise(async (resolve, reject) => {
            try {
                const dagsArray = dags.dags.split(',');
                const url = process.env.REDCAP_HOST + process.env.REDCAP_PATH;
                const data = `token=${this.token}&content=dag&dags=${dagsArray}&format=json&returnFormat=json`;
                const config = {
                    headers: {
                        'Content-Type': `application/x-www-form-urlencoded`,
                        'Accept': 'application/json'
