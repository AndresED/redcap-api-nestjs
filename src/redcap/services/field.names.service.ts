...
dotenv.config();
@Injectable()
export class FieldNamesRedcapService {
    token: string;
    constructor(
    ...
    import(fileData: UploadFileDto) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const data = `token=${this.token}&data=${xlData}&content=fieldname&format=json&returnFormat=json`;
                const config = {
                    headers: {
    ...
    export() {
        return new Promise(async (resolve, reject) => {
            try {
                const url = process.env.REDCAP_HOST + process.env.REDCAP_PATH;
                const data = `token=${this.token}&content=fieldname&format=json&returnFormat=json`;
                const config = {
                    headers: {
                        'Content-Type': `application/x-www-form-urlencoded`,
                        'Accept': 'application/json'
    ...
    delete(fieldnames: FieldNamesDeleteDto) {
        return new Promise(async (resolve, reject) => {
            try {
                const fieldnamesArray = fieldnames.fieldnames.split(',');
                const url = process.env.REDCAP_HOST + process.env.REDCAP_PATH;
                const data = `token=${this.token}&content=fieldname&fieldnames=${fieldnamesArray}&format=json&returnFormat=json`;
                const config = {
                    headers: {
                        'Content-Type': `application/x-www-form-urlencoded`,
                        'Accept': 'application/json'
